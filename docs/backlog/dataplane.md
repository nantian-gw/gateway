...
- Report now has a reload / xDS apply section that aggregates observations, average, p95/p99 bucket upper bound, and sum by `stage` from the `aether_gateway_dataplane_xds_apply_stage_duration_ms` histogram in `metrics.prom`, for locating apply-stage jitter in reload-under-load evidence.
...
- Data plane `/metrics` now exposes endpoint recovery latency histogram `aether_gateway_dataplane_endpoint_recovery_latency_ms`, and inherits cumulative observations across snapshot runtime state; throughput reports can directly consume recovery latency p95 / p99.
...
- `2026-05-06` Added low-cardinality duration histogram for core xDS apply stages, recording duration for `decode`, `inherit_runtime_state`, `snapshot_swap`, `listener_apply`, `ack_wait` stages, and outputting `aether_gateway_dataplane_xds_apply_stage_duration_ms{stage=...}` bucket/sum/count at `/metrics`.
...
- Request latency histogram uses only low-cardinality labels by default. `/metrics` already outputs `aether_gateway_dataplane_traffic_request_latency_ms` with labels limited to `listener`, `protocol`, `route_kind`, `status_class`, `response_flag`.
- Route/backend level high-cardinality metrics are fixed in `/v1/traffic` admin summary; `aether_gateway_dataplane_traffic_*` metrics at `/metrics` do not output route/backend/pod/endpoint level labels.
