...
func TestInfrastructureEndpointSupportsFilteringSortingAndPagination(t *testing.T) {
	...
	if got := infrastructureResourceKeys(report.Resources); strings.Join(got, ",") != "default/echo,default/"+infrastructure.GatewayServiceName("public") {
	...
	if got := infrastructureResourceKeys(report.Resources); strings.Join(got, ",") != "default/"+mesh.ShadowServiceName("default", "stale")+",default/"+infrastructure.GatewayServiceName("public") {
